...
XInput_GetStateGuideButton(UserIndex) {
	...
	*/
	...
	if (status := DllCall(_XInput_GetSecretState, "uint",UserIndex, "uint",&xiSecretState)) {
		Throw "Error getting Guide button state: " status
	}
...
; Example: Control the vibration motors using the analog triggers of each controller. Also runs onLeftStickPress() and onRightStickPress() functions when pressed left/right analog stricks repsectively
